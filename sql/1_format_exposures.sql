-- convert cohort_id to condition_concept_ids
CREATE TABLE #cohort_to_condition (cohort_id INT, ancestor_concept_id INT)

INSERT INTO #cohort_to_condition VALUES
(500000401,	197320),
(500000401,	432961),
(500000301,	201065),
(500000301,	200451),
...
CREATE INDEX cohort_id ON #cohort_to_condition (cohort_id);
CREATE INDEX conds ON #cohort_to_condition (ancestor_concept_id);

-- Get cases only
-- Check reference_ades for patients who experienced the events of interest

SELECT DISTINCT co.person_id as person_id
     , COALESCE(DEATH_DATE, CAST('2017-12-31' as date)) as last_date
INTO #cases
FROM concept_ancestor ca
INNER JOIN #cohort_to_condition sd on
    ca.ancestor_concept_id = sd.ancestor_concept_id
INNER JOIN CONDITION_OCCURRENCE co on
    co.CONDITION_CONCEPT_ID = ca.descendant_concept_id
LEFT JOIN death on
    death.PERSON_ID = co.PERSON_ID

create index person_id
    on #cases (person_id);

SELECT count( distinct person_id) FROM #cases;

-- ONLY DRUG CONDITION PAIRS FROM REFERENCE SET --
...
-- drug_concept_id: drug name
-- drug_start: start of drug exposure
-- drug_end: end of drug exposure - if no end date then start date is used (+90 days)
-- exclude patients who have drug exposures after their death date
-- start exposures at 30 years of age

-- need: person_id, DRUG_CONCEPT_ID, drug_start, drug_end

DROP TABLE IF EXISTS results.ade_drug
SELECT DISTINCT #cases.person_id as person_id
     , DRUG_ERA.DRUG_CONCEPT_ID as DRUG_CONCEPT_ID
     , DRUG_ERA_START_DATE as drug_start
     , CASE
        WHEN DATEADD(year, 1, DRUG_ERA_END_DATE) > last_date THEN last_date
        ELSE DATEADD(year, 1, DRUG_ERA_END_DATE)
    END as drug_end
     , last_date
INTO results.ade_drug
FROM #cases
INNER JOIN DRUG_ERA on
    DRUG_ERA.PERSON_ID = #cases.person_id
INNER JOIN #reference_set rs on
    DRUG_ERA.DRUG_CONCEPT_ID = rs.drug_concept_id

-- run reformat_marketscan.R